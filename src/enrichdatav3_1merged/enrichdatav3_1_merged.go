		...
		if line[emailColumnIndex] != "" { // if email IS provided
			...
			if i == 0 { // first line
				...
				if (userInfo == UserInfo{}) { // record found
					...
					var userInfoConversations = searchCumulativeYearLogs(db, moyEndDate, email, true)
					...
					var userInfoCurrentYearConversations = searchCurrentYearLogs(db, startDate, moyEndDate, email, true)
					...
					var userInfoCurrentYearLogs = searchCurrentYearLogs(db, startDate, moyEndDate, email, false)
