...
func main() {
	if len(os.Args) < 8 {
		...
		fmt.Println("CORRECT Usage: enrichdata3_1merged_bmeoy_dates {fileName} {startDateStr} {boyEndDate} " +
			"{moyEndDate} {eoyEndDate} {emailColumnIndex} {columnCountIndex}")
		fmt.Println("")
		...
		fmt.Println("{boyEndDate} - BOY end date range - (YYYY-mm-dd)")
		fmt.Println("{moyEndDate} - MOY end date range - (YYYY-mm-dd)")
		fmt.Println("{eoyEndDate} - EOY end date range - (YYYY-mm-dd)")
	...
	fmt.Println("boyEndDateStr: ", boyEndDateStr)
	fmt.Println("moyEndDateStr: ", moyEndDateStr)
	fmt.Println("eoyEndDateStr: ", eoyEndDateStr)

	boyEndDateStamp := getTimeStampStringFromDateString(boyEndDateStr, false)
	moyEndDateStamp := getTimeStampStringFromDateString(moyEndDateStr, false)
	eoyEndDateStamp := getTimeStampStringFromDateString(eoyEndDateStr, false)

	fmt.Println("boyEndDateStamp: ", boyEndDateStamp)
	fmt.Println("moyEndDateStamp: ", moyEndDateStamp)
	fmt.Println("eoyEndDateStamp: ", eoyEndDateStamp)

	value := convertStrToDate(startDateStr)
	t, _ := time.Parse(layout, value)
	startDate := strconv.FormatInt(t.Unix(), 10)

	enrichData(fileName, startDate, boyEndDateStamp, moyEndDateStamp, eoyEndDateStamp, emailColumnIndex, columnCount)
}
...
func convertStrToDateStandard(str string) string {
	...
	if len(strArr) > 2 {
		...
		//fmt.Println("Date format must be in mm/dd/YY")
		return retStr
	} else {
		fmt.Println("Date format must be in YYYY-mm-dd")
		return ""
...
func enrichData(fileName string, startDate string, boyEndDateStamp string, moyEndDateStamp string,
	eoyEndDateStamp string, emailColumnIndex int64, columnCount int) {
